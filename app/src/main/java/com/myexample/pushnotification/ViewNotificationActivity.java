...
public class ViewNotificationActivity extends Activity {
    ...
    TextView head, sub,id;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.view_notification);
        head = (TextView)findViewById(R.id.data1);
        sub = (TextView)findViewById(R.id.data2);
        id = (TextView)findViewById(R.id.data3);

        Bundle getBundle = getIntent().getExtras();
        if(getBundle !=null){
            head.setText(getBundle.getString("data1",""));
            sub.setText(getBundle.getString("data2",""));
            id.setText(getBundle.getString("data3",""));
        }

        SharedPreferences sharedpreferences = ViewNotificationActivity.this.getSharedPreferences("Notification", Context.MODE_PRIVATE);
        final String regId= sharedpreferences.getString("regId","");
        /*activate.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {

                if(new CommonClass().isInternetConnected(ViewNotificationActivity.this)){
                    if(!regId.equals(""))
                            new CommonClass().displayToast(ViewNotificationActivity.this, "Your device is already activated.Please click \"Send SMS\" button to communicate with server.");
                    else{
                        ...
                        ViewNotificationActivity.this.startService(registerIntent);
//                PendingIntent{14e1ab57: android.os.BinderProxy@294a644}
                    }
                }else
                    new CommonClass().displayToast(ViewNotificationActivity.this, "Internet is required.");
            }
        });
            ...
            public void onClick(View view) {
                ...
                if (new CommonClass().isInternetConnected(ViewNotificationActivity.this)) {
                    if (sharedpreferences.getString("regId", "").equals("")) {
                        new CommonClass().displayToast(ViewNotificationActivity.this, "Please activate your device by clicking \"Activate\" button.");
                    } else {
...
*/
    ...
    public void callAlertBoxToSendSMS(final String mobileNumber,final String msg) {
        ...
        LayoutInflater inflater = ViewNotificationActivity.this.getLayoutInflater();
        ...
        yesButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View arg0) {
                dialognew.dismiss();
                TelephonyManager tm = (TelephonyManager) ViewNotificationActivity.this
                        .getSystemService(Context.TELEPHONY_SERVICE);
                if (tm.getSimState() != TelephonyManager.SIM_STATE_ABSENT) { // TO get the mobile number list
                    sendNetworkSMS(mobileNumber, msg);
                }else
                    new CommonClass().displayToast(ViewNotificationActivity.this, "Please insert a SIM.");
    ...
    public synchronized void sendNetworkSMS(final String no, String msg) {
        try {
            ...
            PendingIntent sentPI = PendingIntent.getBroadcast(ViewNotificationActivity.this, 0,
                    sentIntent, PendingIntent.FLAG_UPDATE_CURRENT);

            Intent deliveryIntent = new Intent(DELIVERED);

            PendingIntent deliverPI = PendingIntent.getBroadcast(ViewNotificationActivity.this, 0,
                    deliveryIntent, PendingIntent.FLAG_UPDATE_CURRENT);
            /* Register for SMS send action */
            ViewNotificationActivity.this.registerReceiver(new BroadcastReceiver() {
            ...
            Toast.makeText(ViewNotificationActivity.this, ex.getMessage().toString(),
                    Toast.LENGTH_LONG).show();
            ex.printStackTrace();
        }
    }
